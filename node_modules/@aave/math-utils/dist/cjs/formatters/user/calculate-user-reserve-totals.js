...
function calculateUserReserveTotals({ userReserves, userEmodeCategoryId, }) {
    ...
    userReserves.forEach(userReserveSummary => {
        ...
        totalBorrowsMarketReferenceCurrency =
            totalBorrowsMarketReferenceCurrency.plus(userReserveSummary.variableBorrowsMarketReferenceCurrency);
        if (userReserveSummary.userReserve.reserve.reserveLiquidationThreshold !==
            '0' &&
            ...
            const selectedEModeCategory = userReserveSummary.userReserve.reserve.eModes.find(elem => elem.id === userEmodeCategoryId);
            if (userEmodeCategoryId &&
                selectedEModeCategory &&
                selectedEModeCategory.collateralEnabled) {
                currentLtv = currentLtv.plus((0, bignumber_1.valueToBigNumber)(userReserveSummary.underlyingBalanceMarketReferenceCurrency).multipliedBy(selectedEModeCategory.eMode.ltv));
                currentLiquidationThreshold = currentLiquidationThreshold.plus((0, bignumber_1.valueToBigNumber)(userReserveSummary.underlyingBalanceMarketReferenceCurrency).multipliedBy(selectedEModeCategory.eMode.liquidationThreshold));
            }
            else {
