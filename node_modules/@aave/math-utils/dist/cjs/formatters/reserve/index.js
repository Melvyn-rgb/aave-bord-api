...
function getComputedReserveFields({ reserve, currentTimestamp, }) {
    const { totalDebt, totalVariableDebt, totalLiquidity } = (0, calculate_reserve_debt_1.calculateReserveDebt)(reserve, currentTimestamp);
    ...
    const availableLiquidity = reserve.borrowCap === '0'
        ...
        totalDebt.plus(1)));
    const supplyAPY = (0, calculate_compounded_interest_1.calculateCompoundedRate)({
        rate: reserve.liquidityRate,
        duration: constants_1.SECONDS_PER_YEAR,
    });
    const variableBorrowAPY = (0, calculate_compounded_interest_1.calculateCompoundedRate)({
 ...
 */
function formatEnhancedReserve({ reserve, eModes, }) {
    ...
    const formattedEModes = eModes
        ? (0, emode_1.getAndFormatReserveEModes)(reserve.originalId, eModes)
        : [];
    return Object.assign(Object.assign({}, reserve), { totalVariableDebt: normalizeWithReserve(reserve.totalVariableDebt), totalLiquidity: normalizeWithReserve(reserve.totalLiquidity), formattedAvailableLiquidity: normalizeWithReserve(reserve.availableLiquidity), unborrowedLiquidity: normalizeWithReserve(reserve.unborrowedLiquidity), borrowUsageRatio: reserve.borrowUsageRatio, supplyUsageRatio: reserve.supplyUsageRatio, totalDebt: normalizeWithReserve(reserve.totalDebt), formattedBaseLTVasCollateral: (0, bignumber_1.normalize)(reserve.baseLTVasCollateral, index_1.LTV_PRECISION), reserveFactor: (0, bignumber_1.normalize)(reserve.reserveFactor, index_1.LTV_PRECISION), supplyAPY: (0, bignumber_1.normalize)(reserve.supplyAPY, constants_1.RAY_DECIMALS), supplyAPR: (0, bignumber_1.normalize)(reserve.liquidityRate, constants_1.RAY_DECIMALS), variableBorrowAPY: (0, bignumber_1.normalize)(reserve.variableBorrowAPY, constants_1.RAY_DECIMALS), variableBorrowAPR: (0, bignumber_1.normalize)(reserve.variableBorrowRate, constants_1.RAY_DECIMALS), formattedReserveLiquidationThreshold: (0, bignumber_1.normalize)(reserve.reserveLiquidationThreshold, 4), formattedReserveLiquidationBonus: (0, bignumber_1.normalize)((0, bignumber_1.valueToBigNumber)(reserve.reserveLiquidationBonus).minus(10 ** index_1.LTV_PRECISION), 4), totalScaledVariableDebt: normalizeWithReserve(reserve.totalScaledVariableDebt), debtCeilingUSD: isIsolated
 ...
 */
function formatReserveUSD({ reserve, currentTimestamp, marketReferencePriceInUsd, marketReferenceCurrencyDecimals, eModes, }) {
...
function formatReserves({ reserves, eModes, currentTimestamp, marketReferencePriceInUsd, marketReferenceCurrencyDecimals, }) {
...
function formatReservesAndIncentives({ reserves, eModes, currentTimestamp, marketReferencePriceInUsd, marketReferenceCurrencyDecimals, reserveIncentives, }) {
